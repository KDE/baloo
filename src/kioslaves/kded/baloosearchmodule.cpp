...
void SearchModule::init()
{
    ...
    connect(m_dirNotify, SIGNAL(enteredDirectory(QString)),
            this, SLOT(registerSearchUrl(QString)));
    connect(m_dirNotify, SIGNAL(leftDirectory(QString)),
            this, SLOT(unregisterSearchUrl(QString)));


    // FIXME: Listen to changes from Baloo!!
    // Listen to dbChanged
    QDBusConnection con = QDBusConnection::sessionBus();
    con.connect(QString(), QLatin1String("/files"), QLatin1String("org.kde.baloo"),
                QLatin1String("updated"), this, SLOT(slotBalooFileDbChanged()));
